...
CREATE TABLE IF NOT EXISTS companies (
    ...
    embedding_id    TEXT,
    scrape_strategy  TEXT,       -- JSON: ScrapeStrategy generated by discovery agent
    strategy_status  TEXT DEFAULT 'active'  -- active | disabled | failed
);
"""
...
def init_db(db_path: Path) -> sqlite3.Connection:
    ...
    company_cols = {row["name"] for row in cur.fetchall()}
    if "scrape_strategy" not in company_cols:
        conn.execute("ALTER TABLE companies ADD COLUMN scrape_strategy TEXT")
    if "strategy_status" not in company_cols:
...
def get_company(conn: sqlite3.Connection, company_id: int) -> dict | None:
    ...
    if not row:
        return None
    d = _row_to_dict(row)
    if d.get("scrape_strategy"):
        try:
            d["scrape_strategy"] = json.loads(d["scrape_strategy"])
        except (json.JSONDecodeError, TypeError):
            d["scrape_strategy"] = None
    return d


def get_company_by_name(conn: sqlite3.Connection, name: str) -> dict | None:
    cur = conn.execute("SELECT * FROM companies WHERE name = ?", (name,))
    row = cur.fetchone()
    if not row:
        return None
    d = _row_to_dict(row)
...
def upsert_company(conn: sqlite3.Connection, data: dict) -> int:
    ...
    strategy = data.get("scrape_strategy")
    strategy_json = json.dumps(strategy) if isinstance(strategy, dict) else (strategy or None)
    strategy_status = data.get("strategy_status", "active")

    if row:
        company_id = int(row["id"])
        fields: dict[str, object] = {}
        for k in (
            "domain", "ats_type", "careers_url", "ats_slug",
            "tech_stack", "location", "hq_city", "size_category",
            "company_type", "funding_stage", "embedding_id",
        ):
            if k in data and data[k] is not None:
                fields[k] = data[k]
        if strategy is not None:
            fields["scrape_strategy"] = strategy_json
        if "strategy_status" in data:
            fields["strategy_status"] = strategy_status
        if fields:
            update_company_fields(conn, company_id, fields)
        return company_id
    ...
    cols = [
        ...
        "scrape_strategy", "strategy_status",
    ]
    insert_fields: dict[str, object] = {k: data[k] for k in cols if k in data and data[k] is not None}
    insert_fields["name"] = name
    if "scrape_strategy" not in insert_fields and strategy_json:
        insert_fields["scrape_strategy"] = strategy_json
    if "strategy_status" not in insert_fields:
        insert_fields["strategy_status"] = strategy_status
...
def _row_to_dict(row: sqlite3.Row) -> dict:
    d = dict(row)
    if "scrape_strategy" in d and d["scrape_strategy"]:
        try:
            d["scrape_strategy"] = json.loads(d["scrape_strategy"])
        except (json.JSONDecodeError, TypeError):
            pass
